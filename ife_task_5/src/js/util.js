define(function() {
    ...
    function isNull(arg) {
        return protoToString.call(arg) === DataType.Null;

        // 一种不用等号的方法
        // var reg = new RegExp('^\\[object Null\\]$');
        // return reg.test(protoToString.call(arg));

        // return /^\[object Null\]$/.test(protoToString.call(arg));
    }
    ...
    function cloneObject(src) {
        ...
        if (!src || isString(src) || isNumber(src) || isBool(src)) { //基本值 null, string, number, bool
            ...
            for (var item in src) { //用forin遍历数组或对象是为了访问扩展的属性（不是通过索引，类似对象属性）
                if (src.hasOwnProperty(item) && !isFunction(src[item]) && !isRegExp(src[item])) { //防止继承属性和函数和正则
                    newValue[item] = cloneObject(src[item]); //还是用key的方式保持原样(允许有关联数组）
                }
            } //end for
    ...
    function hasClass(element, classStr) {
        if (!element || !element.className || !classStr) {
            return false;
        }
    ...
    function removeClass(element, oldClass) {
        if (!element || !element.className || !oldClass) {
            return;
        }
        //要么以字符串开头或结尾，要么字符串前后都有1或多个空格
        var re = new RegExp('(^|\\s+)' + oldClass + '(\\s+|$)', 'g');
        element.className = element.className.replace(re, '$2'); //$2会根据被删除的类名后面是否有空格替换为0个或1个空格
    }
    ...
    function $(selector, element) {
        ...
        var startNode = element || document;
        ...
        })(startNode, 0); //起点节点从document开始，selectorIndex从0开始
